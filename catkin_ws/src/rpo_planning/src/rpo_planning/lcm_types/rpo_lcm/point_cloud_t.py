...
import rpo_lcm.header_t

class point_cloud_t(object):
    __slots__ = ["header", "num_points", "points"]

    __typenames__ = ["rpo_lcm.header_t", "int32_t", "rpo_lcm.point_t"]

    __dimensions__ = [None, None, ["num_points"]]

    def __init__(self):
        self.header = rpo_lcm.header_t()
        self.num_points = 0
        self.points = []
    ...
    def _encode_one(self, buf):
        assert self.header._get_packed_fingerprint() == rpo_lcm.header_t._get_packed_fingerprint()
        self.header._encode_one(buf)
        buf.write(struct.pack(">i", self.num_points))
        for i0 in range(self.num_points):
            assert self.points[i0]._get_packed_fingerprint() == rpo_lcm.point_t._get_packed_fingerprint()
    ...
    def _decode_one(buf):
        self = point_cloud_t()
        self.header = rpo_lcm.header_t._decode_one(buf)
        self.num_points = struct.unpack(">i", buf.read(4))[0]
        self.points = []
        for i0 in range(self.num_points):
            self.points.append(rpo_lcm.point_t._decode_one(buf))
        return self
    ...
    def _get_hash_recursive(parents):
        ...
        tmphash = (0xef9b8c517c4acf37+ rpo_lcm.header_t._get_hash_recursive(newparents)+ rpo_lcm.point_t._get_hash_recursive(newparents)) & 0xffffffffffffffff
