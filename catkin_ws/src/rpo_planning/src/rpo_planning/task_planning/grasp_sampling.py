...
from rpo_planning.utils import ros_visualize as visualize_helper
from rpo_planning.utils.planning.collision import is_collision
...
class GraspSampling(object):
    ...
    def generate_grasp_samples(self, num_samples=100, point_dist_tol=0.002,
        ...
        for counter, face_pair in enumerate(grasp_index_list):
            for sample in range(0, num_samples):
                ...
                point = helper.point_on_triangle(faces[face_pair[0]])
                ...
                in_triangle = helper.point_in_triangle(projected_point, faces[face_pair[1]])
                ...
                if flag=="collisions":
                        for grasp_id in range(len(self.samples_dict['gripper_poses'][i])):
                            ...
                            wrist_to_tip = util.list2pose_stamped([0.0, 0.071399, -0.14344421, 0.0, 0.0, 0.0, 1.0],
                                                                       '')
                            wrist_left = util.convert_reference_frame(wrist_to_tip,
                                                                           util.unit_pose(),
                                                                           gripper_pose_left,
                                                                           "proposals_base")
                            wrist_right = util.convert_reference_frame(wrist_to_tip,
                                                                            util.unit_pose(),
                                                                            gripper_pose_right,
                                                                            "proposals_base")
    ...
    def initialize_stable_placements(self):
        ...
        for placement_id in range(len(self.sampler.object.stable_placement_dict['T'])):
            ...
            object_pose = util.pose_from_matrix(T,
                                                     frame_id="yumi_body")
            ...
            for point_list, normal_list, sample_id in zip(self.dict_grasp['points'], self.dict_grasp['face_normals'], self.dict_grasp['sample_ids']):
                ...
                for angle in angle_list:
                    ...
                    gripper_left_gripper_left = util.convert_reference_frame(pose_list[0],
                                                                                  pose_list[0],
                                                                                  util.unit_pose(),
                                                                                  frame_id = "gripper_left")
                    gripper_right_gripper_left = util.convert_reference_frame(pose_list[1],
                                                                                   pose_list[0],
                                                                                   util.unit_pose(),
                                                                                   frame_id="gripper_left")
                    ...
                    pose_rotation_gripper_left = util.pose_from_matrix(T_pose_rotation_gripper_left,  'gripper_left')
                    gripper_left_rotated_gripper_left = util.transform_pose(gripper_left_gripper_left, pose_rotation_gripper_left)
                    gripper_right_rotated_gripper_left = util.transform_pose(gripper_right_gripper_left, pose_rotation_gripper_left)
                    #iii. convert back to proposals_base
                    gripper_left_rotated_proposals_base = util.convert_reference_frame(gripper_left_rotated_gripper_left,
                                                                                            util.unit_pose(),
                                                                                            pose_list[0],
                                                                                            frame_id="proposals_base")
                    gripper_right_rotated_proposals_base = util.convert_reference_frame(gripper_right_rotated_gripper_left,
                                                                                            util.unit_pose(),
                                                                                             pose_list[0],
                                                                                   frame_id="proposals_base")
    ...
    def visualize_placement(self, id, proposals=None):
        ...
        pose_object_world = util.pose_from_matrix(T_object_world)
    ...
    def check_collisions(self, gripper_pose_list):
        wrist_to_tip = util.list2pose_stamped([0.0, 0.071399, -0.14344421, 0.0, 0.0, 0.0, 1.0], '')
        wrist_left_proposal = util.convert_reference_frame(wrist_to_tip,
                                                       util.unit_pose(),
                                                       gripper_pose_list[0],
                                                       "proposals_base")
        wrist_right_proposal = util.convert_reference_frame(wrist_to_tip,
                                                        util.unit_pose(),
                                                        gripper_pose_list[1],
                                                        "proposals_base")
        wrist_left_world = util.convert_reference_frame(wrist_left_proposal,
                                                       util.unit_pose(),
                                                       self.sampler.base_initial,
                                                       "yumi_body")
        wrist_right_world = util.convert_reference_frame(wrist_right_proposal,
                                                        util.unit_pose(),
                                                        self.sampler.base_initial,
                                                        "yumi_body")
        self.sampler.gripper_left.setCollisionPose(wrist_left_world)
        self.sampler.gripper_right.setCollisionPose(wrist_right_world)
        collision_list = []
        collision_list.append(is_collision(self.sampler.gripper_left.collision_object,
                                            self.sampler.table.collision_object))
        collision_list.append(is_collision(self.sampler.gripper_right.collision_object,
                                                   self.sampler.table.collision_object))
...
def gripper_from_proposal( grasp_point_list, grasp_normal_list):
    ...
    for point, normal in zip(grasp_point_list, grasp_normal_list):
        ...
        quat = util.mat2quat(hand_orient_norm)
        #define hand pose
        pose_list.append(util.list2pose_stamped(list(point) + list(quat)))
...
def grasp_from_proposal(point_list_new, normal_list_new, T, sampling_base_pose):
    ...
    for pose in pose_list:
        ...
        #                                                  pose)
        pose_stamped_world = util.convert_reference_frame(pose,
                                                               util.unit_pose(),
                                                               sampling_base_pose,
                                                                "yumi_body")
    ...
    for gripper_pose_world in pose_stamped_list_aligned:
        ...
        #                                                                         gripper_pose_world)
        gripper_pose_body = util.convert_reference_frame(gripper_pose_world,
                                                               sampling_base_pose,
                                                              util.unit_pose(),
                                                              "sampling_base")
        T_gripper_body = util.matrix_from_pose(gripper_pose_body)
        ...
        pose_new_list.append(util.pose_from_matrix(T_gripper_body_new,
                                                        frame_id="proposals_base"))
