...
def main(args):
    ...
    for filename in os.listdir(full_data_dir):
        ...
        if filename != 'metadata.pkl':
            #print(filename)
	    #continue
            with open(os.path.join(full_data_dir, filename, 'pkl', filename+'.pkl'), 'rb') as f:
                try:
			data = pickle.load(f)
		except EOFError:
			continue
            ...
            data['keypoints_start'] = np.matmul(start_mat, keypoints_world_homog.T).T[:, :3]
            data['keypoints_goal_corrected'] = np.matmul(goal_mat, keypoints_world_homog.T).T[:, :3]
            ...
            with open(os.path.join(full_data_dir, filename, 'pkl', filename+'.pkl'), 'wb') as new_f:
