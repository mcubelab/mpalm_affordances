...
RUN mkdir -p ${HOME}/environments && cd ${HOME}/environments && virtualenv -p `which python3.6` py36-gnn

RUN PATH=/usr/local/cuda/bin:$PATH && \
    CPATH=/usr/local/cuda/include:$CPATH && \
    LD_LIBRARY_PATH=/usr/local/cuda/lib64:$LD_LIBRARY_PATH

# takes care of either installing pytorch geometric or deep graph library, depending on build args
COPY ./install_scripts/install_gnn_lib.sh /tmp/install_gnn_lib.sh
RUN chmod +x /tmp/install_gnn_lib.sh
RUN /tmp/install_gnn_lib.sh
