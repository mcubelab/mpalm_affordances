...
class RGBDCamera(Camera):
    ...
    def get_pcd(self, in_world=True, filter_depth=True,
                ...
                force_rgb=None, force_depth=None):
        """
        ...
        Args:
            ...
            depth_max (float): maximum depth value. If None, it will use the
                default maximum depth value defined in the config file.
            force_rgb (np.ndarray): separately captured RGB image to use in pcd
            force_depth (np.ndarray): separately captured depth image to use in pcd
        ...
        """
        if force_depth is None or force_rgb is None:
            rgb_im, depth_im = self.get_images(get_rgb=True, get_depth=True)
        else:
            rgb_im, depth_im = force_rgb, force_depth
